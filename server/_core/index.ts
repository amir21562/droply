...
async function startServer() {
  ...
  app.post("/api/session/upload", (req, res) => { try { const item = addAnonymousItem(req.body); res.json(item); } catch (error: any) { res.status(error?.code === "FORBIDDEN" ? 403 : 400).json({ message: error?.message || "Upload rejected" }); } });
