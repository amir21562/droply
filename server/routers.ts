...
type Session = { id: string; code: string; createdAt: number; expiresAt: number; participants: Participant[]; items: SharedItem[]; senderId: string };
...
export function leaveAnonymousParticipant(sessionId: string, participantId: string) { const session = sessions.get(sessionId); if (!session) return; session.participants = session.participants.filter((p: Participant) => p.id !== participantId); if (session.participants.length === 0) sessions.delete(session.id); }
...
function cleanSessions() { const now = Date.now(); Array.from(sessions.entries()).forEach(([id, session]: [string, Session]) => { if (session.expiresAt <= now || session.participants.length === 0) sessions.delete(id); }); }
