...
type SharedItem = { id: string; kind: "file" | "text" | "link"; name: string; mimeType?: string; size: number; data?: string; createdAt: number; sender: string; downloads: number };
...
export function updateAnonymousDownload(token: string, status: "complete" | "error" | "cancelled") { const record = downloadTokens.get(token); if (!record) return; if (record.status === "complete") return; record.status = status; if (status === "complete") { try { const session = sessions.get(record.sessionId); const item = session?.items.find((candidate: SharedItem) => candidate.id === record.itemId); if (session && item && record.participantId !== session.senderId) item.downloads += 1; } catch { /* session already gone */ } } }
...
export const appRouter = router({
  ...
  session: router({
    ...
    removeItem: publicProcedure.input(z.object({ sessionId: z.string(), participantId: z.string(), itemId: z.string() })).mutation(({ input }) => removeAnonymousItem(input)),
    ...
    switchRole: publicProcedure.input(z.object({ sessionId: z.string(), participantId: z.string(), targetRole: z.enum(["sender", "receiver"]) })).mutation(({ input }) => { const session = findSession(input); const me = session.participants.find((p: Participant) => p.id === input.participantId); if (!me) throw new TRPCError({ code: "FORBIDDEN", message: "You are not in this room." }); if (input.targetRole === "sender") { session.senderId = me.id; } else if (me.id === session.senderId) { const successor = session.participants.find((p: Participant) => p.id !== me.id); if (!successor) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Wait for someone to join before switching to receiver." }); session.senderId = successor.id; } /* already a receiver -> no-op */ for (const p of session.participants) p.role = p.id === session.senderId ? "sender" : "receiver"; return { session: publicSession(session) }; }),
    destroy: publicProcedure.input(z.object({ sessionId: z.string(), participantId: z.string() })).mutation(({ input }) => { const session = findSession(input); if (input.participantId !== session.senderId) throw new TRPCError({ code: "FORBIDDEN", message: "Only the sender can delete this room." }); sessions.delete(session.id); return { success: true }; }),
  }),
});
