...
export const appRouter = router({
  ...
  session: router({
    ...
    switchRole: publicProcedure.input(z.object({ sessionId: z.string(), participantId: z.string() })).mutation(({ input }) => { const session = findSession(input); const me = session.participants.find((p: Participant) => p.id === input.participantId); if (!me) throw new TRPCError({ code: "FORBIDDEN", message: "You are not in this room." }); session.senderId = me.id; for (const p of session.participants) p.role = p.id === me.id ? "sender" : "receiver"; return { session: publicSession(session) }; }),
